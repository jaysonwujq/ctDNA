...
for my $pair (@pairs) {
  ...
  if ( $first_mate && $second_mate ) {
    ...
    if ( ( $start1 > 0 ) && ( $start2 > 0 ) ) {
      ...
      if ( ( $reads1 =~ /^ACGT..T/ ) && ( $strand1 == 1 ) ) {
        $index1 = substr( $reads1, 4, 2 );
      }
      elsif ( ( $reads1 =~ /A..ACGT$/ ) && ( $strand1 == -1 ) ) {
        $index1 = reverse( substr( $reads1, $reads1_length - 6, 2 ) );
        $index1 =~ tr/ATCG/TAGC/;
      ...
      if ( ( $reads2 =~ /^ACGT..T/ ) && ( $strand2 == 1 ) ) {
        $index2 = substr( $reads2, 4, 2 );
      }
      elsif ( ( $reads2 =~ /A..ACGT$/ ) && ( $strand2 == -1 ) ) {
        $index2 = reverse( substr( $reads2, $reads2_length - 6, 2 ) );
        $index2 =~ tr/ATCG/TAGC/;
