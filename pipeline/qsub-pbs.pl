...
my (
  $Queue,    $Interval,   $Lines,   $Maxjob, $Convert, $Secure, $Reqsub,
  $Resource, $Job_prefix, $Verbose, $Help
);
GetOptions(
  "lines:i"     => \$Lines,
  "maxjob:i"    => \$Maxjob,
...
$Maxjob     ||= 20;
$Convert    ||= 'yes';
$Resource   ||= "nodes=1:ppn=1,mem=15G,walltime=1000:00:00";
$Job_prefix ||= "work";
...
my $Work_dir    = File::Spec->rel2abs( $work_shell_file . ".$$.qsub" );
my $current_dir = `pwd`;
chomp $current_dir;
...
while (<IN>) {
  ...
  if ( $line_mark % $Lines == 0 ) {
    open OUT, ">$Work_dir/$Job_prefix\_$Job_mark.sh"
      || die "failed creat $Job_prefix\_$Job_mark.sh";
    push @Shell, "$Job_prefix\_$Job_mark.sh";
    $Job_mark++;
  }
  s/;\s*$//
    ; ##delete the last character ";", because two ";;" characters will cause error in qsub
  s/;\s*;/;/g;
...
while (@Shell) {
  ...
  my $job_cmd = "qsub -d $Work_dir ";    ## -l h_vmem=16G,s_core=8
  ...
  for ( my $i = 0; $i < @Shell; $i++ ) {
    while (1) {
      ...
      if ( $i < $Maxjob || ( $run_num != -1 && $run_num < $Maxjob ) ) {
        ...
        print "$job_return";
        my $job_id = $1
          if ( $job_return =~ /(\d+)\.mu01/ )
          ;    #300021.torque01.int.bioinfo.ucr.edu
...
sub run_count {
  ...
  foreach my $job_line (@jobs) {
    ...
    my $jobtemp = $1 if ( $job_field[0] =~ /(\d+)\.mu01/ );
  ...
  return
    $run_num
    ; ##qstat结果中的处于正常运行状态的任务，不包含那些在已死掉节点上的僵尸任务
}
